...
function renderKatagoriPelajaran() {
  ...
  const semuaDataKategoriPelajaran = CategoryManager.getKategoriPelajaranFromLocalStorage();
  ...
  semuaDataKategoriPelajaran.forEach((data) => {
    ...
    <button id="delete-${data.category_id}" class="btn btn-danger">Delete</button>
    </td>
    `;
...
function editKategoriPelajaran(id) {
  ...
  const editDataKategoriPelajaran =
  semuaDataKategoriPelajaran.filter(
    (data) => data.category_id === id
  );
  
  document.getElementById("oldId").value = editDataKategoriPelajaran[0].category_id;
  document.getElementById("name").value = editDataKategoriPelajaran[0].name_category;   
